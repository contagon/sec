...
class lasso(cyipopt.Problem):
    def __init__(self, A, y):
        ...
        super(lasso, self).__init__(2 * self._m, 2 * self._m, cl=cl)
    ...
    def objective(self, x):
        ...
        return np.linalg.norm(
            self._y - np.dot(self._A, w)
        ) ** 2 / 2 + self._lambda * np.sum(u)
    ...
    def gradient(self, x):
        ...
        g = np.vstack(
            (
                np.dot(-self._A.T, self._y - np.dot(self._A, w)),
                self._lambda * np.ones((self._m, 1)),
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--plot", help="Plot results with matplotlib", action="store_true"
    )
